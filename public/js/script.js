	...
	function attachFormValidator(elements) {
		...
		var regularConstraintsMessages = [
			...
			},
			{
				...
				newMessage: "L'adresse mail n'est pas valide."
			},
			{
				type: regula.Constraint.Numeric,
				newMessage: "Seulement des nombres peuvent être écrits"
			},
			{
...
/*
  ...
  document.addEventListener('DOMContentLoaded', function () {
    var today = new Date();
    [].slice.call(document.querySelectorAll('#airbnb label[for^=date-]')).forEach(e => {
    ...
    document.getElementById('airbnb').addEventListener('submit', function(e) {
      e.preventDefault();
      var url = this.getAttribute('action')+'?';
      var data = new FormData(this);
      for(var entry of data) {
        if(!/(check_in|check_out|adults)/.test(entry[0])) continue;
        if(entry[1].toString().trim().length<1) return;
        url += '&'+entry[0]+'='+entry[1];
      }
      location.href = url;
    });
...
function checkContactForm() {
  ...
  for(var entry of data) {
    if(entry[1].toString().trim().length<1) return false;
  }
  return true;
}
...
function sendmail(token) {
  if(checkContactForm()) {
    var data = {
      ...
      subject: $("#contact form select[name='subject']").val(),
      message: $("#contact form textarea[name='message']").val()
    };
    $.ajax({
      type: "POST",
      url: "/.netlify/functions/send-mail",
      data: data,
      success: function(){
        $('.fielddata').parent().addClass('sent');
      }
    });
  }
}
